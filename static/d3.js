...
const bgcol = cssvar('--bg-col');
const fgcol = cssvar('--fg-col');
const fg2col = cssvar('--fg2-col');
...
function Tree(data, { // data is either tabular (array of objects) or hierarchy (nested objects)
  ...
  fill = fgcol, // fill for nodes
  fillOpacity, // fill opacity for nodes
  stroke = fg2col, // stroke for links
  strokeWidth = 1.5, // stroke width for links
  strokeOpacity = 0.4, // stroke opacity for links
  strokeLinejoin, // stroke line join for links
  strokeLinecap, // stroke line cap for links
  halo = bgcol, // color of label halo 
...
function radial_tree(editor, dataobj) {
  ...
  svg.append('g')
    ...
    .attr('transform', d => `
                  ...
                  translate(${d.y},0)
                `)
    .attr('fill', d => d.children ? fg2col : fgcol)
    .attr('r', 2.5);
...
function force_tree(editor, dataobj, {
  ...
  const texts = svg.append("g")
    ...
    .data(nodes)
    .join("text")
    .attr("style", "user-select: none; font-family: 'Roboto Mono', monospace; font-size: 0.5rem;")
    .attr("fill", d => d.data.children || d.data.more ? fg2col : fgcol)
    .attr("dy", "0.32em")
    .attr("dx", 12)
  ...
  const node = svg.append("g")
    ...
    .data(nodes)
    .join("circle")
    .attr("fill", d => d.data.children ? (d.data.toplevel ? fgcol : null) : "#000")
    .attr("stroke", d => d.data.children ? (d.data.toplevel ? fg2col : null) : "#fff")
    .attr("stroke-dasharray", d => d.data.more ? "1,1" : null)
    .attr("r", 3.5)
    .on("click", (d, i) => {
